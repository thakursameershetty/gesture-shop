...
import { ShoppingBag, Hand, MousePointer2, ChevronUp, ChevronDown, Trash2, Eye } from 'lucide-react';
...
export default function App() {
  ...
  const { videoRef, cursor: handCursor, isGrabbing: isHandGrabbing, isModelLoaded } = useHandTracking();
  ...
  // --- NEW: SYNTHESIZER SOUND ENGINE (No Files Required) ---
  // --- UPDATED SOUND ENGINE (Volume Reduced) ---
  const playSynthSound = (type) => {
    ...
    if (type === 'grab') {
      ...
      // "POP OUT" (Release Sound)
      osc.type = 'sine';
      osc.frequency.setValueAtTime(300, ctx.currentTime);
      osc.frequency.exponentialRampToValueAtTime(600, ctx.currentTime + 0.1);

      // Keeping drop slightly louder so you hear the confirmation
  ...
  useEffect(() => {
    ...
    // 3. DRAG START
    if (isGrabbing && !draggedProduct && activeCursor.y >= 0.15 && activeCursor.y <= 0.85) {
      const elements = document.elementsFromPoint(x, y);
      const cardElement = elements.find(el => el.getAttribute('data-product-id'));
      if (cardElement) {
        const id = parseInt(cardElement.getAttribute('data-product-id'));
        const product = PRODUCTS.find(p => p.id === id);
        if (product && !cart.find(c => c.id === id)) {

          // --- TRIGGER GRAB SOUND ---
          playSynthSound('grab'); // <--- Uses math to generate sound instantly
    ...
    if (!isGrabbing && draggedProduct) {
      if (cartZoneRef.current) {
        ...
        if (x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom) {
          ...
          // --- TRIGGER DROP SOUND ---
          playSynthSound('drop'); // <--- Uses math to generate sound instantly

          setCart(prev => [...prev, draggedProduct]);
        }
      }
      setDraggedProduct(null);
    }
  }, [isGrabbing, activeCursor, draggedProduct, cart]);
    ...
    <div className="w-screen h-screen overflow-hidden relative font-sans flex flex-col md:flex-row text-gray-800 bg-transparent">
      ...
      <div className={`fixed inset-0 z-[-1] bg-gray-100 transition-colors duration-500 ${inputMode === 'gesture' ? 'bg-black' : ''}`}>
        <video
          ...
          className={`w-full h-full object-cover transform -scale-x-100 transition-opacity duration-700 ${inputMode === 'gesture' ? 'opacity-100' : 'opacity-0'}`}
        />
      </div>
      ...
      <div ref={scrollContainerRef} className="w-full md:w-2/3 h-full p-6 pt-24 md:p-10 overflow-y-auto scroll-smooth no-scrollbar select-none relative">
        ...
        <header className="hidden md:flex mb-8 justify-between items-center bg-white/80 backdrop-blur-xl p-4 rounded-[32px] shadow-sm border border-white/20">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900">New Drops</h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {inputMode === 'gesture' && (
              <div className="flex items-center gap-2 px-4 border-r border-gray-300">
                ...
                <input type="range" min="0" max="100" value={bgBlur} onChange={(e) => setBgBlur(e.target.value)} className="w-24 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                <span className="text-xs font-mono w-10 text-right">{bgBlur}px</span>
              </div>
            )}
            {/* Toggles */}
            <div className="flex bg-gray-100 rounded-full p-1">
              <button onClick={() => setInputMode('mouse')} className={`px-4 py-2 rounded-full flex items-center gap-2 text-sm font-semibold transition-all ${inputMode === 'mouse' ? 'bg-black text-white' : 'text-gray-500'}`}><MousePointer2 size={16} /> Mouse</button>
              <button onClick={() => setInputMode('gesture')} className={`px-4 py-2 rounded-full flex items-center gap-2 text-sm font-semibold transition-all ${inputMode === 'gesture' ? 'bg-[#00e5ff] text-black' : 'text-gray-500'}`}><Hand size={16} /> Gesture AI</button>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-48 md:pb-40">
          {PRODUCTS.map(product => {
            const isHidden = draggedProduct?.id === product.id || cart.find(c => c.id === product.id);
            return (
        ...
        {/* Visual Zones */}
        <div className={`fixed top-0 left-0 w-full md:w-2/3 h-32 bg-gradient-to-b from-black/10 to-transparent pointer-events-none transition-opacity duration-300 flex justify-center pt-8 md:pt-4 ${activeZone === 'TOP' ? 'opacity-100' : 'opacity-0'}`}>
          <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold shadow-sm h-10 animate-bounce"><ChevronUp /> Scroll Up</div>
        </div>
        <div className={`fixed bottom-0 left-0 w-full md:w-2/3 h-32 bg-gradient-to-t from-black/10 to-transparent pointer-events-none transition-opacity duration-300 flex justify-center items-end pb-32 md:pb-4 ${activeZone === 'BOTTOM' ? 'opacity-100' : 'opacity-0'}`}>
          <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold shadow-sm h-10 animate-bounce"><ChevronDown /> Scroll Down</div>
        </div>
      </div>
      ...
      <div ref={cartZoneRef} className={`fixed bottom-0 left-0 right-0 h-auto max-h-[30vh] md:max-h-none md:relative md:w-1/3 md:h-full border-t md:border-t-0 md:border-l border-white/30 p-6 md:p-8 transition-all duration-300 z-30 ${draggedProduct ? 'bg-blue-50/90' : 'bg-white/60'} backdrop-blur-xl shadow-2xl`}>
        <div className="flex items-center justify-between mb-4 md:mb-8">
          ...
          <div className="flex gap-4">
            ...
            <span className="bg-black text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">{cart.length}</span>
          </div>
        </div>
          ...
          <AnimatePresence>
            {cart.length === 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-500 border-2 border-dashed border-gray-400/50 rounded-[24px]">
                <p className="hidden md:block">Drag items here</p>
                <p className="md:hidden">Drop items here</p>
              </div>
            )}
            {cart.map((item, index) => (
              <motion.div key={item.id} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="md:absolute md:top-0 md:left-0 md:right-0 min-w-[80px] md:min-w-0" style={{ zIndex: index, marginTop: window.innerWidth > 768 ? index * 60 : 0 }}>
                <div className="hidden md:flex bg-white/90 backdrop-blur border border-white/50 rounded-[24px] p-4 shadow-lg items-center gap-4">
                  <img src={item.image} className="w-20 h-20 rounded-xl object-cover" />
                  <div><h3 className="font-bold">{item.name}</h3><p className="text-gray-500">${item.price}</p></div>
                </div>
                <div className="md:hidden w-20 h-20 rounded-xl overflow-hidden border border-gray-200 shadow-sm">
        ...
        <div className="hidden md:block absolute bottom-8 left-8 right-8">
          <div className="flex justify-between text-lg font-bold mb-4"><span>Total</span><span>${cart.reduce((acc, item) => acc + item.price, 0)}</span></div>
